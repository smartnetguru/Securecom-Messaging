...
public class SecurecomMessagingSmsListener extends BroadcastReceiver {
  ...
  private boolean isChallenge(Context context, Intent intent) {
    ...
    if (messageBody.matches("Your Securecom messaging verification code: [0-9]{3,4}-[0-9]{3,4}") &&
  ...
  public void onReceive(Context context, Intent intent) {
    Log.w("SecurecomMessagingSmsListener", "Got SMS broadcast...");

    if (SMS_RECEIVED_ACTION.equals(intent.getAction()) && isChallenge(context, intent)) {
      Log.w("SecurecomMessagingSmsListener", "Got challenge!");
