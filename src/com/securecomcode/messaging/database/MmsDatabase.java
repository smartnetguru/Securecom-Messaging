...
public class MmsDatabase extends Database implements MmsSmsColumns {
      ...
      throws MmsException
  {
    ...
    boolean       unread        = com.securecomcode.messaging.util.Util.isDefaultSmsProvider(context) ||
  ...
  public Pair<Long, Long> insertMessageInbox(NotificationInd notification) {
    try {
      ...
      contentValues.put(STATUS, Status.DOWNLOAD_INITIALIZED);
      contentValues.put(DATE_RECEIVED, System.currentTimeMillis() / 1000);
      contentValues.put(READ, com.securecomcode.messaging.util.Util.isDefaultSmsProvider(context) ? 0 : 1);
...
//      Trimmer.trimThread(context, threadId);
  ...
  public void markIncomingNotificationReceived(long threadId) {
    ...
    if (com.securecomcode.messaging.util.Util.isDefaultSmsProvider(context)) {
      DatabaseFactory.getThreadDatabase(context).setUnread(threadId);
    }
