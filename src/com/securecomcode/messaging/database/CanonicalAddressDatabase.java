...
public class CanonicalAddressDatabase {
  ...
  private static final String DATABASE_CREATE  = "CREATE TABLE " + TABLE + " (" + ID_COLUMN + " integer PRIMARY KEY, " + ADDRESS_COLUMN + " TEXT NOT NULL UNIQUE);";
  ...
  private static final String EMAIL_SELECTION  = ADDRESS_COLUMN + " = ?";
  ...
  private long getCanonicalAddressFromDatabase(String address) {
    ...
    try {
      ...
      if(Util.isValidEmail(address)){
          cursor                      = db.query(TABLE, ID_PROJECTION, EMAIL_SELECTION, selectionArguments, null, null, null);
      }else{
