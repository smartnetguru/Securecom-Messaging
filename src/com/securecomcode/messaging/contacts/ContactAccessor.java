 ...
 */
package com.securecomcode.messaging.contacts;
...
public class ContactAccessor {
    ...
    public Collection<ContactData> getContactsWithPush(Context context) {
        ...
        final String[] emailProjection = new String[]{ContactsContract.CommonDataKinds.Email.CONTACT_ID, ContactsContract.Contacts.DISPLAY_NAME};
        ...
        for (String pushNumber : pushNumbers) {
            if (org.whispersystems.textsecure.internal.util.Util.isValidEmail(pushNumber)) {
                Uri uri = Uri.withAppendedPath(ContactsContract.CommonDataKinds.Email.CONTENT_FILTER_URI, pushNumber);
                lookupCursor = resolver.query(uri, emailProjection, null, null, null);
            } else {
                Uri uri = Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, Uri.encode(pushNumber));
                lookupCursor = resolver.query(uri, inProjection, null, null, null);
    ...
    public ContactData getContacInfo(Context context, Uri uri) {
        ...
        try {
            numberCursor = context.getContentResolver().query(ContactsContract.CommonDataKinds.Email.CONTENT_URI, null,
                    ContactsContract.CommonDataKinds.Email.CONTACT_ID + " = ?",
                    new String[]{contactData.id + ""}, null);

            while (numberCursor != null && numberCursor.moveToNext()) {
                int type = numberCursor.getInt(numberCursor.getColumnIndexOrThrow(ContactsContract.CommonDataKinds.Email.TYPE));
                String label = numberCursor.getString(numberCursor.getColumnIndexOrThrow(ContactsContract.CommonDataKinds.Email.LABEL));
                String number = numberCursor.getString(numberCursor.getColumnIndexOrThrow(ContactsContract.CommonDataKinds.Email.DATA));
                String typeLabel = ContactsContract.CommonDataKinds.Email.getTypeLabel(context.getResources(), type, label).toString();
